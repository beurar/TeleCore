...
public class NetworkInfoView
{
    ...
    private void SetExtensions()
    {
        ...
        {
            ExtendoTabs.Add("Requester Settings", delegate(Rect rect) { parentComp.RequestWorker.DrawSettings(rect); });
        }
        ...
        if (parentComp.NetworkRole.HasFlag(NetworkRole.Storage))
        {
            ExtendoTabs.Add("Filter Settings", delegate(Rect rect)
            {
                ...
                var scrollViewRect = new Rect(listingRect.x, listingRect.y + listing.curY, listingRect.width,
                    (parentComp.Container.AcceptedTypes.Count + 1) * Text.LineHeight);
    ...
    public void DrawMainContent(Rect rect)
    {
        ...
        if ((parentComp.NetworkRole & NetworkRole.Requester) == NetworkRole.Requester)
        {
            ...
            if (Widgets.ButtonText(selectorRect, parentComp.RequestWorker.Mode.ToStringSafe()))
            {
                FloatMenu menu = new FloatMenu(new List<FloatMenuOption>()
                {
                    new(RequesterMode.Automatic.ToStringSafe(),
                        delegate { parentComp.RequestWorker.SetMode(RequesterMode.Automatic); }),
                    new(RequesterMode.Manual.ToStringSafe(),
                        delegate { parentComp.RequestWorker.SetMode(RequesterMode.Manual); }),
                }, "Set Mode", true);
                menu.vanishIfMouseDistant = true;
                Find.WindowStack.Add(menu);
            }
            ...
            TWidgets.FloatRange(requestSliderRect, parentComp.GetHashCode(), ref requestRange, 0.01f, 1f,
