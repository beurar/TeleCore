...
public static class TeleExposeUtility
{
    ...
    {
        return TeleSerializeUtility.SerializeUInt(map.info.NumCells, (idx) => intReader(map.cellIndices.IndexToCell(idx)));
    }
        
    public static void LoadUInt(byte[] arr, Map map, Action<IntVec3, uint> intWriter)
    {
        TeleSerializeUtility.LoadUInt(arr, map.info.NumCells, delegate(int idx, uint data)
        {
    ...
    public static void ExposeUInt(Map map, Func<IntVec3, uint> uintReader, Action<IntVec3, uint> uintWriter, string label)
    {
        ...
        {
            arr = SerializeUInt(map, uintReader);
        }
        DataExposeUtility.ByteArray(ref arr, label);
        if (Scribe.mode == LoadSaveMode.LoadingVars)
        {
            LoadUInt(arr, map, uintWriter);
        }
    }
}