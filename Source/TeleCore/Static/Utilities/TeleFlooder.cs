...
namespace TeleCore
{
    ...
    {
        private static readonly HashSet<IntVec3> _PassedCells = new HashSet<IntVec3>();
        private static readonly Queue<IntVec3> _OpenSet = new Queue<IntVec3>();
        ...
        public static IEnumerable<IntVec3> Flood(Map map, IntVec3 originCell, Action<IntVec3> processor, Predicate<IntVec3> validator, int maxCells = 9999)
        {
        ...
        /// </summary>
        public static IEnumerable<IntVec3> Flood(Map map, CellRect originRect, Action<IntVec3> processor, Predicate<IntVec3> validator, int maxCells = 9999)
        {
            int num = -1;
            _PassedCells.Clear();
            _OpenSet.Clear();
            foreach (var cell in originRect.Cells)
            {
                _OpenSet.Enqueue(cell);
            }
            while (_OpenSet.Count > 0)
            {
                ...
                if (num >= maxCells)
                    break;
                IntVec3 curCell = _OpenSet.Dequeue();
                ...
                foreach (var adjCell in curCell.CellsAdjacent8Way().Where(c => c.InBounds(map) && !_PassedCells.Contains(c) && validator.Invoke(c)).InRandomOrder())
                {
                    _OpenSet.Enqueue(adjCell);
                }
            }
        }
    }
