...
namespace TeleCore
{
        ...
        public override void PostSpawnSetup(bool respawningAfterLoad)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                foreach (var animation in set.animations)
                {
                    ...
                    {
                        var animationKeyFrame = animation.keyFrames[k];
                        var list = animationKeyFrame.savedList ?? new List<KeyFrame>();
    ...
    public class CompProperties_AnimationRenderer : CompProperties
    {
        ...
        public void LoadDef()
        {
            ...
            {
                if (f.Extension == ".xml" && f.Name == $"{animationDefName}Def.xml")
                {
                    Scribe.loader.InitLoading(f.FullName);
                    try
                    {
                        if (!Scribe.EnterNode(nameof(AnimationDataDef)))
                        {
                        ...
                        animationDef = new AnimationDataDef();
                        animationDef.ExposeData();
            ...
            TLog.Message($"Loaded AnimationDef: {animationDef != null}: {animationDef?.defName}");
        }
    }
}
